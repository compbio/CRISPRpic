...
def main(args):
    ...
    print "Inital Index Size :" + str(index_size)
    ...
    for i, row in df_list.iterrows():
        ...
        if os.path.exists(locus):
            ...
            print locus + "\t" + "amplicon is very low complex" + "\n"
...
def enz_info(input_file,window_size,index_size,log_file):
    print >>sys.stderr, "Checking reference sequence and gRNAseqs"
    ...
    with open (input_file) as f:
        for lines in f:
            ...
            if len(split_seq) > 2:
                print "gRNAseq site is not unique"
            else:
                ...
                if enz_type == 1:   # SpCas9
                    ...
                    print "Error, please indicate the type of enzyme"
                    sys.exit()
                ...
                if motif_len_up > 15 or motif_len_down > 15:
                    print "very low complex sequences"
...
def get_amplicon(locus,amplicon):
    print >>sys.stderr, "selecting amplicons"
...
def make_mutseq(locus,window_size,upstr,downstr,enz_type):
    print >>sys.stderr, "making possible mutant sequences"
...
def mut_freq(locus,upstr,downstr,window_size,motif_len_up,motif_len_down,enz_type):
    print >>sys.stderr, "counting the frequency of mutations"
    if enz_type == 2:
        window_size += 2
    target1 = upstr[20:]
    target2 = downstr[:-20]
...
def freq_summary(locus,upstr,downstr):
    print >>sys.stderr, "summarizing frequencies"
...
def plot_freq(locus):
    print >>sys.stderr, "plotting figures"
    index = []
    label = []
    freq = []
    count = 1
